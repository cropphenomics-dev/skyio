...
class CloudPath(AnyPath):
    ...
    def __init__(self, cloud_path: str, *args, **kwargs):
        ...
        # Recalculate local_path to initialize the Path object correctly
        # This is required for Python 3.12+ where Path.__init__ must be called with args
        path_netloc = self.uri_parts.netloc
        cache_folder = get_cache_folder()
        local_path = cache_folder.joinpath(
            path_netloc, self.uri_parts.path.lstrip("/")
        )
        self._local_path = local_path
        
        if sys.version_info >= (3, 12):
            super().__init__(str(local_path))
        else:
            super().__init__()
